...
`
  ...
  //  if (process.platform !== 'linux') throw new Error('must be run from linux')
   ...
   ///home/kridai/code/twilio-cli/dist/deb
    ...
    const build = async (arch) => {
      ...
      //'twilio_2.32.1-1_amd64'
      const versionedDebBase = `twilio_${debVersion}_${debArch(arch)}` 
      // '/home/kridai/code/twilio-cli/tmp/apt/twilio_2.32.1-1_amd64.apt'
    ...
    const gpgKey = process.env.DEB_KEY
    const passphrase = process.env.DEB_KEY_PASSPHRASE
    if (gpgKey) {
